...
export const onRequest = async (context: any) => {
  ...
  try {
    ...
    const expectedUser = ((env && env.ADMIN_USERNAME) || process.env.ADMIN_USERNAME || "zyqro87").toLowerCase();
    const expectedPass = (env && env.ADMIN_PASSWORD) || process.env.ADMIN_PASSWORD || "digital97@-";
    const expectedSecCode = (env && env.ADMIN_SECURITY_CODE) || process.env.ADMIN_SECURITY_CODE || "2005";
