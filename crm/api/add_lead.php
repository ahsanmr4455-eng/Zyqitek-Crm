...
$id = $input['id'] ?? ('lead-' . round(microtime(true) * 1000));
$name = $input['name'] ?? '';
$email = $input['email'] ?? '';
$phone = $input['phone'] ?? '';
...
try {
    $stmt = $db->prepare("INSERT INTO leads (id, name, email, phone, company, status, value, source, category, notes, instagramLink, facebookLink, linkedinLink, websiteUrl, otherLink, smmPlatformName, smmPlannedPosts, smmPostingFrequency, smmContentNotes, smmCampaignRequirements, assignedTeamMember, createdAt, updatedAt) 
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $id,
        $name,
    ...
    echo json_encode(["success" => false, "error" => "Failed to add lead: " . $e->getMessage()]);
}
